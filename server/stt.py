"""Speech-to-text for hold-to-talk audio.
...
def stt_enabled() -> bool:
    if os.environ.get("PLUSONE_STT", "1").strip().lower() in ("0", "false", "no", "off"):
        return False
...
async def transcribe_wav(wav_bytes: bytes, *, timeout: float = 30.0) -> str:
    ...
    key = muse_api_key()
    if not key:
        raise RuntimeError("STT off: MUSE_API_KEY / MUSE_TRANSCRIBE_KEY is not set")
    if not wav_bytes:

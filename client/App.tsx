...
export default function App() {
  ...
  const endHold = async (channel: 'public' | 'private') => {
    ...
    const ms = statusNow.durationMillis ?? 0;
    ...
    const visibility = channel === 'public' ? 'public' : `private:${who}`;
    const pending = text.trim() ? text.trim() : 'hearing you…';
    setLog((prev) => [...prev, { kind: 'you', visibility, text: pending, pending: !text.trim() }]);
    setHearing(true);
    setHint('hearing you…');
    try {
      ...
      if (text.trim()) {
        ...
        setHint('couldn’t hear that — type it?');
      }
    } finally {
      setHearing(false);
    }
